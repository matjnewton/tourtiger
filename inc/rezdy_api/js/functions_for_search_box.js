// available for search box - disable now
(function () {
    "use strict";
    var wqs_2 = angular.module("wqs_2", ['ngAnimate','angular.filter',]);


// config
    wqs_2.config(['$httpProvider', function( $httpProvider) {
...
// controler
    wqs_2.controller('wqs_search_controller', function ($scope, $http, $q, dataService, dataServiceAjax, dataServiceAjaxmore, $timeout, $filter) {
	...
	// load api key !need load js_var in init.php
        //console.log(js_var.apikey);

    // enable load animations
         $scope.loading = true;

    // load rezdy category !need load js_var in init.php
    	console.log('functions_for_search_box->js_var.rezdy_cat_id'+js_var.rezdy_cat_id);
...
//dataService starttime and endtime get URLparametr
    wqs_2.factory('dataService', function($http, $q, $filter){
        return{
...
// dataServiceAjax for click (!not cleare)
    wqs_2.factory('dataServiceAjax', function($http, $q, $filter){
        return{
            getData: function(productCode,startTime,endTime){
                var deferred = $q.defer();
...
// dataServiceAjaxmore for infinite
    wqs_2.factory('dataServiceAjaxmore', function($http, $q, $filter){
        return{
            getData: function(productCode,startTime,endTime){
                var deferred = $q.defer();
...
/* filters */
	...
	wqs_2.filter("asDate", function ($filter) {
	    return function (input) {
	        return $filter('date')(new Date(input), 'yyyy-MM-dd', 'UTC');
	    }
	});
 	wqs_2.filter("asTime", function ($filter) {
	    return function (input) {
	        return $filter('date')(new Date(input), 'hh:mm a', 'UTC');
	    }
	});
	wqs_2.filter("asDateTitle", function ($filter) {
	    return function (input) {
	        return $filter('date')(new Date(input), 'dd MMMM', 'UTC');
	    }
	});
	wqs_2.filter("trust", ['$sce', function($sce) {
	...
	wqs_2.directive("otcScripts", function() {
