...
<div class="col-lg-3 col-md-6 col-xs-12 form-group">
 <font class='DEPARTURE_FLIGHT_DT'> *</font>
  ...
  <input type='text' readonly required name="dep_fl_dt" class="form-control DEPARTURE_FLIGHT_DT_LABEL"/><span class="btn-primary input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
  </div>
 </div>
 ...
 <div class='input-group date' id='dep_fl_tm'>
  <input type='text' readonly required name="dep_fl_tm" required class="form-control DEPARTURE_FLIGHT_TM_LABEL"/><span class="btn-primary input-group-addon"><span class="glyphicon glyphicon-time"></span></span>
 </div>
</div>
...
flag_dt = 0;
  ...
  j(function () {
    ...
    j('#dep_fl_dt').datetimepicker({ignoreReadonly: true, minDate: h, format: 'DD/MM/YYYY', locale: 'en-gb'});
...
return;
  ...
  j('.middle').show();
  dataValue=j( this ).serialize()+"&prod="+id_prods+"&label="+j('#all-classes').val()+"&action=11&oneway="+j('input[type=radio]:checked').val()+"&promo="+j('#promo-code').val()+"&from="+j('#all-places').val()+"&to="+j('#match-places').val()+"&pax="+j('#all-classes option:selected').text()+"&pp="+pp;
j.ajax({
      type: "POST",
...
purchase ="<form class='twowayairport'><h5 class='uppercase'>"+j('.YOUR_ORDER').val()+"</h5><label>"+j('.FROM').val()+"</label> "+arr[0].from+"<input type='hidden' name='from' value ='"+arr[0].from+"'><br><label>"+j('.TO').val()+"</font></label> "+arr[0].to+"<input type='hidden' name='to' value ='"+arr[0].to+"'><br><label>"+j('.PASSENGERS').val()+"</label> "+arr[0].pax+"<input type='hidden' name='pax' value ='"+arr[0].pax+"'><br><label>"+j('.RETURN').val()+"</label> "+arr[0].oneway+"<input type='hidden' name='oneway' value ='"+arr[0].oneway+"'><br><label>Promo:</label> "+arr[0].promo+"<input type='hidden' name='promo' value ='"+arr[0].promo+"'><br><label>Total:</label> "+parseFloat(arr[0].total).toFixed(2)+"€ <input type='hidden' name='pp' value ='"+arr[0].total+"'><br><h5>"+j('.DEPARTURE_DETAILS').html()+"</h5><label>"+j('.DEPARTURE_FLIGHT_N').text()+":</label> "+arr[0].voo_n_dep+" <input type='hidden' name='voo_n_dep' value ='"+arr[0].voo_n_dep+"'><br><label>"+j('.DEPARTURE_FLIGHT_DT').text()+":</label> "+arr[0].voo_dep_dt+"<input type='hidden' name='voo_dep_dt' value ='"+arr[0].voo_dep_dt+"'><br><label>"+j('.DEPARTURE_FLIGHT_TM').text()+":</label> "+arr[0].voo_dep_tm+"<input type='hidden' name='voo_dep_tm' value ='"+arr[0].voo_dep_tm+"'><br><label>"+j('.DEPARTURE_FLIGHT_TR_TIME').text()+":</label> "+arr[0].voo_dep_pick+"<input type='hidden' name='voo_dep_pick' value ='"+arr[0].voo_dep_pick+"'><br><label>"+j('.DEPARTURE_FLIGHT_ADDR').text()+":</label> "+arr[0].voo_dep_mor+"<br><input type='hidden' name='voo_dep_mor' value ='"+arr[0].voo_dep_mor+"'><h5>"+j('.PERSONAL_DETAILS').html()+"</h5><label>"+j('.PERSONAL_DETAILS_NAME').text()+":</label> "+arr[0].nome+"<input type='hidden' name='nome' value ='"+arr[0].nome+"'><br><label>"+j('.PERSONAL_DETAILS_EMAIL').text()+":</label> "+arr[0].email+"<input type='hidden' name='email' value ='"+arr[0].email+"'><br><label>"+j('.PERSONAL_DETAILS_TEL').text()+":</label> "+arr[0].tel+"<input type='hidden' name='tel' value ='"+arr[0].tel+"'><br><label>"+j('.PERSONAL_DETAILS_COUNTRY').text()+":</label> "+arr[0].country+"<input type='hidden' name='country' value ='"+arr[0].country+"'><br><h5>"+j('.NR_PAX').html()+"</h5><label>"+j('.ADULT').text()+":</label> "+arr[0].adult+"<input type='hidden' name='adult' value ='"+arr[0].adult+"'><br><label>"+('.CHILDREN').text()+":</label> "+arr[0].children+"<input type='hidden' name='children' value ='"+arr[0].children+"'><br><label>"+j('.BABY').text()+":</label> "+arr[0].baby+"<input type='hidden' name='baby' value ='"+arr[0].baby+"'><br><h5>"+j('.OBS').html()+"</h5><label>"+j('.OBS_TXT').text()+":</label> "+arr[0].obs+"<input type='hidden' name='obs' value ='"+arr[0].obs+"'><br><h5>"+j('.PAYMENTS_MET').html()+"</h5><label>"+j('.PAYMENTS_MET_LABEL').text()+":</label> "+pay+"<input type='hidden' name='payments' value ='"+arr[0].payments+"'><br><input type='hidden' value='"+arr[0].id+"' name='prod'><input type='hidden' value='"+arr[0].id_price+"' name='label'><input type='hidden' value='"+arr[0].lang+"' name='language'></form>";
