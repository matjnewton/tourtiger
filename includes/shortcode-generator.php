	...
	function gdl_testimonial_shortcode( $atts ){
		...
		if( $type == 'static' ){
			...
			foreach( $testimonials as $testimonial ){
				...
				$url = get_post_meta( $testimonial->ID, "testimonial-option-url-comment", true );
				$image = get_post_meta( $testimonial->ID, "testimonial-option-url-image", true );
				
				//image
				$temp = $temp . '<a href="'.$url.'" target="_blank"><img style="float:left;padding-left:20px;padding-right:8px;" src="'.$image.'" alt="testimonial"/></a>';

				// testimonial content
				$temp = $temp . '<div class="testimonial-content"><a href="'.$url.'" target="_blank">';
