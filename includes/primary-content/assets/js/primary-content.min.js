...
function aload(nodes) {
  ...
  [].forEach.call(nodes, function (node) {
    var attrName = node.tagName !== 'LINK' ? 'src' : 'href';

    node[ attrName ] = node.getAttribute(attribute);
    node.removeAttribute(attribute);

    if ( node.tagName === 'IMG' && node.getAttribute('data-aload-set') ) {
      node.setAttribute('srcset', node.getAttribute('data-aload-set'));
    }
  });
  ...
  var methods = {
    ...
    init: function () {
      ...
      if (typeof $.fn.magnificPopup !== 'undefined') {
        ...
        $('.gallery--onclick__popup').magnificPopup({
          delegate: 'a',
          type: 'image',
      ...
      if ( $videoPopup.length > 0 ) {
        $('.video-popup').on('click', '.video-popup--cover', function(){
          ...
          $popup
          .clone()
          .prepend('<a href="javascript:" class="close-popup"><div class="close-sign">X</div></a>')
          .appendTo('body')
          .fadeIn()
          .on(
            'click',
      ...
      if ( $(".pc--date").not('[data-inited]') ) {
        $(".pc--date").each(function(){
          ...
          $(this).daterangepicker({
            locale: {
              format: $(this)[0].dataset.dateFormat.toUpperCase()
            },
      ...
      if ( $('.pc_circle-image--wrapper.js-new-circle').length > 0 ) {
        ...
        $('.pc_circle-image--wrapper.js-new-circle').each(function(){
          ...
          var classL = item.closest('.pc--r')[0].classList.value; //indexOf("welcome")
          var numb = classL.indexOf("pc--r__col-");
          var colunsNumb = classL[numb+11]; // <10 !!!

          var rowWidth = $(item.closest('.pc--r')[0]).width();
          var sectionWidth = rowWidth/colunsNumb;


          // console.debug('Number:', colunsNumb, "Row width:", rowWidth, "Section width:", sectionWidth);
          ...
          if ( item.width() == 0 || item.height() == 0 ) {
            ...
            if ( item.find('img').width() == 0 || item.find('img').height() == 0 ) {
              ...
              if ( item_w < item_h ) {
                ...
                // console.debug("case 3", "h:", item_h, img_h);
                item.parent().width(sectionWidth);
                item.parent().height(sectionWidth);
                item.width(sectionWidth);
                item.height(sectionWidth);
                img.width(sectionWidth);
                img.height(sectionWidth);
              }
              ...
              if ( img_w > img_h ) {
                if ( img_h > item_w ) {
                  ...
                  img.height(item_w).width(item_w);
      ...
      if ( $('.pc--r').find('.pc--crop__thumb').length > 0 ) {
        ...
        $('.pc--r').find('.pc--crop__thumb').each(function(index, thisItem){
          ...
          if (item.hasClass('pc--c__b-image--square')) {
            item.animate( {'max-height': blog_thumb_h, 'height': blog_thumb_h}, 50 );
          } else {
      ...
      var $image = $('img');  // todo: check - this is for data-aload
      $image.each((ind)=>{
        var $img = $($image[ind]);
        if(false && $img.attr('data-image-height') && $img.attr('data-image-width')) {
          $img.css({'height': $img.attr('data-image-height'), 'width' : $img.attr('data-image-width')});
        }
      })
      ...
      $('.pc--form form').submit(function(e){
        ...
        if ($userFields.length > 0) {
          $userFields.each(function(){
            ...
            if ($item.hasClass('pc--date') && value.length) {
              inputValues[name] = value; // handle value
            } else {
